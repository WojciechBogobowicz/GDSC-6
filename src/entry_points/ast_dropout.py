...
class TrainMetricsLogger(transformers.TrainerCallback):
...
if __name__ == "__main__":
    ...
    augmentations = [
        NoiseAug(0.3, noise_ratio=0.001),
        ShiftAug(0.3, 0.08, 'both'),
    ]
...
#     if not args.remote_model_location:
...
#         ckpt_path = get_best_checkpoint_path(local_model_dir)
#         model = transformers.ASTForAudioClassification.from_pretrained(ckpt_path, num_labels=num_labels, label2id=label2id, id2label=id2label, ignore_mismatched_sizes=True)
#         logger.info("Model loaded")
    ...
    #callbacks
    trainer.add_callback(TrainMetricsLogger(trainer))
    trainer.add_callback(transformers.EarlyStoppingCallback(early_stopping_patience = args.patience))
